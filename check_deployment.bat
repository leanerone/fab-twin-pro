...
%WINDIR%\System32\inetsrv\appcmd.exe list sites 2>nul | findstr /i "FabTwin"
if errorlevel 1 (
...
%WINDIR%\System32\inetsrv\appcmd.exe list modules 2>nul | findstr /i "ApplicationRequestRouting" >nul 2>&1
if not errorlevel 1 (
...
echo.
echo [INFO] Checking ARR Proxy...
%WINDIR%\System32\inetsrv\appcmd.exe list config -section:system.webServer/proxy /text:* 2>nul | findstr /i "enabled"
if errorlevel 1 (
...
echo 4. If 401 Unauthorized: run .\deploy_iis.bat to fix auth settings
echo.
pause
